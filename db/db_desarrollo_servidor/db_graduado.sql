...
create table if not exists `criterio` (
 ...
 `cri_nombre_control` varchar(50) not null, 
...
create table if not exists `criterio_pregunta` (
 ...
 `cpre_tipo_control` varchar(1) null, 
 `cpre_orden` int null,  
 `cpre_activo` varchar(1) not null, 
...
create table if not exists `persona_graduado` (
 ...
 `pgra_modalidad` int default null comment '1-> Grado, 2-> Posgrado, 3-> Ambos.',
 ...
 `pgra_estado_logico` varchar(1) not null,
  unique pgra_cedula(pgra_cedula) 
);
-- --------------------------------------------------------
--
...
create table if not exists `info_academico_graduado` (
 ...
 `tins_id` bigint(20) default null,
 `tbac_id` bigint(20) null,
 `jest_id` bigint(20) null,
...
-- --------------------------------------------------------
--
-- Estructura de tabla para la tabla `info_laboral_actual` 
--
create table if not exists `info_laboral_actual` (
 `ilac_id` bigint(20) not null auto_increment primary key,
 `pgra_id` bigint(20) not null,
 `pre_id` bigint(20) not null,
 `cri_id` bigint(20) null,
 `ilac_valor` varchar(500) null, 
 `ilac_estado` varchar(1) not null, 
 `ilac_fecha_creacion` timestamp not null default current_timestamp,
 `ilac_fecha_modificacion` timestamp null default null,
 `ilac_estado_logico` varchar(1) not null, 
 foreign key (pgra_id) references `persona_graduado`(pgra_id),
 foreign key (pre_id) references `pregunta`(pre_id),
 foreign key (cri_id) references `criterio`(cri_id)
);
