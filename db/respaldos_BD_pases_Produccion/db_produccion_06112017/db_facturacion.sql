...
create table if not exists `datos_factura` (
  `dfac_id` bigint(20) not null auto_increment primary key,
  `opag_id` bigint(20) not null,
  `dfac_numero_dni` varchar(15) not null,   
  `dfac_tipo_dni` int not null,    
  `dfac_nombres` varchar(60) not null,
  `dfac_apellidos` varchar(60) not null,   
  `dfac_direccion` varchar(200) not null,   
  `dfac_correo` varchar(50) null,   
  `dfac_telefono` varchar(30) null,   
  `dfac_estado` varchar(1) default null,
  `dfac_fecha_creacion` timestamp not null default current_timestamp,
  `dfac_fecha_modificacion` timestamp null default null,
  `dfac_estado_logico` varchar(1) default null,
  foreign key (opag_id) references `orden_pago` (opag_id)
) ;
