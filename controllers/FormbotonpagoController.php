...
class FormbotonpagoController extends \yii\web\Controller {
    ...
    public function actionBtnpago() {
        ...
        try {
            ...
            $resp_veropago = $mod_opago->verificaordenpago($opag_id); // no se cae
            if ($resp_veropago) {
                $resp_opago = $mod_opago->datosBotonpago($opag_id, 'SI'); // no se cae
                if ($resp_opago) {
        ...
        if (VPOSResponse($arrayIn, $arrayOut, $llaveVPOSFirmaPub, $llaveComercioCryptoPriv, $vector)) {
            ...
            if ($arrayOut['authorizationResult'] == "00") {
                ...
                try {
                    ...
                    $respdatos = $mod_ordpago->consultaNumerotransaccion($numero_transaccion); // no se cae
                    ...
                    //$resp_curso = $mod_ordpago->consultaCurso($resp_sol["metodo_ingreso"], $resp_sol["nivel_interes"]); // TOCA REVISAR SI SE NECESITA POR Q SE CAE NO EXISTE YA UNA TABLA
                    //$fecha = $resp_curso['fecha'];
                            ...
                            if ($banderacrea == 1) {
                                ...
                                if ($resp_desglose) {
                                    ...
                                    if ($resp_actcar) {
                                        ...
                                        if ($resp_opag) {
                                            if ($creg_estado_pago == 'SP') {
                                                $resp_encuentra = $mod_ordpago->encuentraAspirante($int_id); // NO SE CAE
                                                if ($resp_encuentra) {
                                                    $asp = $resp_encuentra['asp_id'];
                                                    $continua = 1;
                                                } else {
                                                    //Se asigna al interesado como aspirante
                                                    $resp_asp = $mod_ordpago->insertarAspirante($int_id); //NO SE CAE
                                                    if ($resp_asp) {
                                                        $asp = $resp_asp;
                                                        $resp_inte = $mod_ordpago->actualizaEstadointeresado($int_id); // NO SE CAE
                                                        if ($resp_inte) {
                                                            $continua = 1;
                                                        }
                                                    }
                                                ...
                                                if ($continua == 1) {
                                                    ...
                                                    //Se obtiene el curso para luego registrarlo.
                                                    if ($resp_sol) {
                                                        //$resp_curso = $mod_ordpago->consultaCurso($resp_sol["metodo_ingreso"], $resp_sol["nivel_interes"]); // SE CAE POR TABLA QUE NO EXITE VER SI SE NECESITA
                                                        //if ($resp_curso) {
                                                        //Registro al curso (temporalmente)
                                                        //$resp_ingcurso = $mod_ordpago->insertarRegistrocurso($resp_curso['cur_id'], $asp); // SE CAE POR UE NECESITA EL CURSO DE ARRIBA
                                                        //if ($resp_ingcurso) {
                                                        //envio de correo                                                                    
                                                        $mod_persona = new Persona();
