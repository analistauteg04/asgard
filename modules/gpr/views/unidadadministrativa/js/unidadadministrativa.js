...
function edit() {
    var link = $('#txth_base').val() + "/gpr/unidadadministrativa/edit" + "?id=" + $("#frm_id").val();
    window.location = link;
}

function update() {
    var link = $('#txth_base').val() + "/gpr/unidadadministrativa/update";
    var arrParams = new Object();
    ...
    if (objContact.length == 0) {
        var msg = objLang.The_Administrative_Unit_must_have_at_least_1_responsible_;
        shortModal(msg, objLang.Error, "error");
        return;
    }
    if (!validateForm()) {
...
function save() {
    var link = $('#txth_base').val() + "/gpr/unidadadministrativa/save";
    var arrParams = new Object();
    arrParams.nombre = $('#frm_name').val();
    arrParams.desc = $('#frm_desc').val();
    arrParams.entidad = $('#cmb_ent').val();
    ...
    if (!validateForm()) {
        requestHttpAjax(link, arrParams, function(response) {
            ...
            if (response.status == "OK") {
                setTimeout(function() {
                    window.location.href = $('#txth_base').val() + "/gpr/unidadadministrativa/index";
                }, 3000);
            }
        }, true);
    }
...
function deleteItem(id) {
    var link = $('#txth_base').val() + "/gpr/unidadadministrativa/delete";
    var arrParams = new Object();
    arrParams.id = id;
    requestHttpAjax(link, arrParams, function(response) {
        if (response.status == "OK") {
