...
<div>
    ...
    PbGridView::widget([
        ...
        [
            ...
            [
                'attribute' => 'Pago',
                'header' => Yii::t("formulario", "Payment"),
                'value' => 'pago',
            ],
            [
                'attribute' => 'Fpago',
                    ...
                    'view' => function ($url, $model) {                        
                        if (($model['estado'] != 'Aprobado') and ($model['usu_preaprueba'] != $model['usu_autenticado'])) {
                            if (!empty($model['sins_fecha_reprobacion'])) {
                                return Html::a('<span class="glyphicon glyphicon-check"></span>', Url::to(['solicitudinscripcion/resultsolaprobar', 'ids' => base64_encode($model['sins_id']), 'int' => base64_encode($model['int_id']), 'perid' => base64_encode($model['persona']), 'apellidos' => base64_encode($model['per_apellidos']), 'nombres' => base64_encode($model['per_nombres']), 'nint_nombre' => base64_encode($model['nint_nombre']), 'car_nombre' => base64_encode($model['car_nombre']), 'fec_repro' => base64_encode($model['sins_fecha_reprobacion']), 'obs_repro' => base64_encode($model['sins_observacion']), 'nint' => base64_encode($model['nint_id'])]), ["data-toggle" => "tooltip", "title" => "Aprobación Solicitud", "data-pjax" => 0]);
                            } else {
